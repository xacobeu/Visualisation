    ...
    const std::vector<std::string>& columns)
{
    ...
    for (const auto& col : columns) {
        ...
        if (series.unit.empty()) {
            if (col.find("_billion_USD") != std::string::npos) series.unit = "billion USD";
            else if (col.find("_million_USD") != std::string::npos) series.unit = "million USD";
            else if (col.find("_USD") != std::string::npos) series.unit = "USD";
            else if (col.find("_percent") != std::string::npos) series.unit = "%";
            else if (col.find("_per_USD") != std::string::npos) series.unit = "per USD";
            else if (col.find("_sq_km") != std::string::npos) series.unit = "sq km";
            else if (col.find("_km") != std::string::npos) series.unit = "km";
        }
...
std::unordered_map<std::string, float> DataHandler::getColumnForAllCountries(const std::string& column) {
    ...
    for (const auto& [country, data] : loadedData) {
        ...
        if (it != data.end() && it->second.isNumeric()) {
            ...
            if (val > 0.0f) result[country] = val;
        }
    }
    return result;
}
...
DataHandler::ColumnData DataHandler::getColumnWithUnitForAllCountries(const std::string& column) {
    ...
    if (result.unit.empty()) {
        if (column.find("_billion_USD") != std::string::npos) result.unit = "billion USD";
        else if (column.find("_million_USD") != std::string::npos) result.unit = "million USD";
        else if (column.find("_USD") != std::string::npos) result.unit = "USD";
        else if (column.find("_percent") != std::string::npos) result.unit = "%";
        else if (column.find("_per_USD") != std::string::npos) result.unit = "per USD";
        else if (column.find("_km") != std::string::npos) result.unit = "km";
        else if (column.find("_sq_km") != std::string::npos) result.unit = "sq km";
        else if (column.find("_Age") != std::string::npos) result.unit = "years";
        else if (column.find("Sex_Ratio") != std::string::npos) result.unit = "(Male : Female)";
        else if (column.find("_kW") != std::string::npos) result.unit = "kW";
        else if (column.find("_bbl_per_day") != std::string::npos) result.unit = "barrels per day";
        else if (column.find("_cubic_meters") != std::string::npos) result.unit = "cubic meters";
        else if (column.find("_Mt") != std::string::npos) result.unit = "million tons";
    }
    return result;
}
...
std::vector<std::string> DataHandler::getAvailableColumns() const {
    ...
    for (const auto& [country, data] : loadedData) {
        for (const auto& [col, val] : data) {
            if (val.isNumeric()) columns.insert(col);
        }
    }
    return std::vector<std::string>(columns.begin(), columns.end());
}

std::vector<std::string> DataHandler::getAllCountryNames() const {
    std::vector<std::string> names;
    names.reserve(loadedData.size());
    for(const auto& [name, _] : loadedData) {
...
DataValue DataHandler::parseValueWithUnit(const std::string& rawValue) {
    if (rawValue.empty()) return DataValue(0.0f, "");
    ...
    for (size_t i = 0; i < cleaned.size(); ++i) {
        ...
        if (c == '-' && !foundNumber && numberPart.empty()) {
            ...
            if (start != std::string::npos) unitPart = unitPart.substr(start);
            break;
        }
    }
    ...
    if (foundNumber && !numberPart.empty() && numberPart != "-") {
        ...
        } catch (...) { return DataValue(rawValue, ""); }
    }
